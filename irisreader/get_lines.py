...
def get_lines( file_object ):
    ...
    elif isinstance( file_object, iris_data_cube ) or isinstance( file_object, sji_cube ) or isinstance( file_object, raster_cube ) or isinstance( file_object, combined_raster ):
    ...
    if fits_data.type == 'sji':
        ...
        wave_field_keys = [k for k in fits_data._fits_file[0].header.keys() if k.startswith("TDET")]
        wave_field_values = [fits_data._fits_file[0].header[x] for x in sorted(wave_field_keys)]
        wave_length_keys = [k for k in fits_data._fits_file[0].header.keys() if k.startswith("TWAVE")]
        wave_length_values = [round(fits_data._fits_file[0].header[x],1) for x in sorted(wave_length_keys)]
        wave_text_keys = [k for k in fits_data._fits_file[0].header.keys() if k.startswith("TDESC")]
        wave_text_values = [fits_data._fits_file[0].header[x] for x in sorted(wave_text_keys)]
