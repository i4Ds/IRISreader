...
from irisreader import iris_data_cube, sji_cube, raster_cube, combined_raster
...
class image_cube_cropper( BaseEstimator, TransformerMixin ):
    ...
    def fit( self, X, y=None ):
        ...
        if isinstance( X, iris_data_cube ) or isinstance( X, sji_cube ) or isinstance( X, raster_cube ) or isinstance( X, combined_raster ):
        ...
        for step in range( self._data_cube_object.n_steps ):
            ...
            except CorruptImageException as e:
                image_bounds.append( [0,0,0,0] )
                self._corrupt_images.append( step )
            
            except NullImageException as e:
                image_bounds.append( [0,0,0,0] )
        ...
        # the inlier images
        self._x_min = np.max( image_bounds[outlier_score < outlier_threshold, 0] )
        self._x_max = np.min( image_bounds[outlier_score < outlier_threshold, 1] )
        self._y_min = np.max( image_bounds[outlier_score < outlier_threshold, 2] )
        self._y_max = np.min( image_bounds[outlier_score < outlier_threshold, 3] )
    ...
    def transform( self, X ):
        """
        Readjusts the hidden image bounds and valid images variables of the supplied data cube.
        ...
        Returns
        -------
        irisreader.iris_data_cube (irisreader.sji_cube / irisreader.raster_cube)
            Data cube object with adjusted valid images and image boundaries (internal variables).
        """
        
        if self._data_cube_object is None:
            raise ValueError("Please pass a data cube first using the fit method.")
        
        data_cube_object_tr = self._data_cube_object.copy()
        
        # remove bad images
        if self._remove_bad:
            data_cube_object_tr._remove_image_steps( self._corrupt_images + self._null_images )

        # set the boundaries
        data_cube_object_tr._set_bounds( self.get_bounds() )
        
        return data_cube_object_tr
