...
class iris_data_cube( object ):
    ...
    def unique_identifiers( self ):
        ...
        """
        ids = []
        dates = [from_Tformat( h['DATE_OBS'] ) for h in self.time_specific_headers]

        for d in dates:      
            if self.type == 'sji':
                line_id = SJI_LINE_WINDOWS.index( self.line_info ) if self.line_info in SJI_LINE_WINDOWS else 0
                fits_type = 1
            else: 
                fits_type = 2
                line_id = RASTER_LINE_WINDOWS.index( self.line_info ) if self.line_info in RASTER_LINE_WINDOWS else 0
